...
An ensemble model approach is used in this paper to generate the submission data set
for the Kaggle __House Price__ competition.  Ensemble modeling involves training 
multiple models and combining their predictions to arrive at the overall 
predictions that are submitted to Kaggle. The specific ensemble approach taken is called model stacking.
...
Extreme Gradient Boosting (xgb) and Random Forest (rngr).
...
__Feature Sets__ in the diagram reprsents one or more sets of features created for the Level 0
models.
...
ggplot() +
  ...
  geom_text(aes(x=1,y=3.5,label="xgb")) +
...
nnet,0.13278,"Level 1"'
),stringsAsFactors = FALSE)

plb_data

pct_reduction <- 100*(1 - filter(plb_data,Model=="nnet")$PLB_Score/filter(plb_data,Model=="xgb")$PLB_Score)
...
Some ways for improving perfomance are

* Additional Level 0 models using different algorithms
* Hyper-parameter tuning
* Improving features set through feature engineering
* Additional levels in the model structure
...
set.seed(13)
data_folds <- createFolds(train.raw$SalePrice, k=2)
...
L1FeatureSet$train$id <- do.call(c,lapply(xgb_set,function(x){x$predictions$ID}))
L1FeatureSet$train$y <- do.call(c,lapply(xgb_set,function(x){x$predictions$y}))
predictors <- data.frame(xgb_yhat,rngr_yhat)
...
L1FeatureSet$test$id <- xgb_submission[,"Id"]
L1FeatureSet$test$predictors <- data.frame(xgb_yhat=test_xgb_yhat,
