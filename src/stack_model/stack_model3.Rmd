---
title: "Ensemble Modeling: Three Level Stack Model Example"
author: "J. Thompson"
date: "September 2016"
...
This paper extends this [Kernel](https://www.kaggle.com/jimthompson/house-prices-advanced-regression-techniques/ensemble-model-stacked-model-example) that illustrates a 2-stage stacked ensemble model to a 3-stage stacked ensemble model.

The diagram below shows the high-level model stacking architecture, which is composed
of three stages. The top-level stage, called Level 0, is composed of
three modeling algorithms:
Gradient Boosting (gbmL0), Extreme Gradient Boosting (xgbL0) and Random Forest (rngrL0).
The second stage, Level 1, is composed of a Neural Network (nnetL1) model and Gradient Boosting (gmbL1).  The 
predictions from Level 1 are blended at Level 2 by geometric weighting.

__Feature Sets__ in the diagram reprsents one or more sets of attributes created for the Level 0
models.

```{r,fig.height=6,fig.width=5,fig.align='center',echo=FALSE,fig.path='figure/'}
...
ggplot() +
  ...
  #Feature Set
  geom_rect(aes(xmin=0.5,ymin=5.5,xmax=4.5,ymax=6),alpha=0.25) +
  geom_text(aes(x=2.5,y=5.75,label="Feature Sets")) +
  geom_segment(aes(x=4,y=5.5,xend=4,yend=5),arrow=prediction_arrow) +
    
  # Feature Set Lines
  geom_segment(aes(x=1,y=5.5,xend=1,yend=5),arrow=prediction_arrow) +
  geom_segment(aes(x=2.5,y=5.5,xend=2.5,yend=5),arrow=prediction_arrow) +
    
    
  # Level 0 Models
  geom_rect(aes(xmin=0.5,ymin=4,xmax=1.5,ymax=5,fill="Level 0")) +
  geom_text(aes(x=1,y=4.5,label="gbmL0")) +
  geom_rect(aes(xmin=2,ymin=4,xmax=3,ymax=5,fill="Level 0")) +
  geom_text(aes(x=2.5,y=4.5,label="xgbL0")) +
  geom_rect(aes(xmin=3.5,ymin=4,xmax=4.5,ymax=5,fill="Level 0")) +
  geom_text(aes(x=4,y=4.5,label="rngrL0")) +
    
  # Prediction Lines Level0 to Level1
  geom_segment(aes(x=1,y=4,xend=3.15,yend=3),arrow=prediction_arrow) +
  geom_segment(aes(x=2.5,y=4,xend=3.25,yend=3),arrow=prediction_arrow) +
  geom_segment(aes(x=4,y=4,xend=3.35,yend=3),arrow=prediction_arrow) +
    
  geom_segment(aes(x=1,y=4,xend=1.65,yend=3),arrow=prediction_arrow) +
  geom_segment(aes(x=2.5,y=4,xend=1.75,yend=3),arrow=prediction_arrow) +
  geom_segment(aes(x=4,y=4,xend=1.85,yend=3),arrow=prediction_arrow) +
    
  #Level 1 Model
  geom_rect(aes(xmin=2.75,ymin=2,xmax=3.75,ymax=3,fill="Level 1")) +
  geom_text(aes(x=3.25,y=2.5,label="nnetL1")) +
    
  geom_rect(aes(xmin=1.25,ymin=2,xmax=2.25,ymax=3,fill="Level 1")) +
  geom_text(aes(x=1.75,y=2.5,label="gbmL1")) +
    
  # Prediction Lines Level1 to Level2
  geom_segment(aes(x=1.75,y=2,xend=2.45,yend=1.5),arrow=prediction_arrow) +
  geom_segment(aes(x=3.25,y=2,xend=2.55,yend=1.5),arrow=prediction_arrow) +
    
  #Level 2 Model
  geom_rect(aes(xmin=2,ymin=0.5,xmax=3,ymax=1.5,fill="Level 2")) +
  geom_text(aes(x=2.5,y=1.0,label="geometric\nblend")) +
  
  # final prediction line
  geom_segment(aes(x=2.5,y=0.5,xend=2.5,yend=0.25),arrow=prediction_arrow) +
  
  geom_text(aes(x=2.5,y=3.5,label="Level 0 Model Predictions",fontface="bold")) +
  geom_text(aes(x=2.5,y=1.75,label="Level 1 Model Prediction",fontface="bold")) +
  geom_text(aes(x=2.5,y=0.20,label="Final Prediction",fontface="bold")) +
...
CARET.TUNE.GRID <-  expand.grid(n.trees=seq(10)*10,
                                interaction.depth=seq(7),
                                shrinkage=c(0.1,0.001),
                                n.minobsinnode=c(5,10,15))  # NULL provides model specific default tuning parameters
...
MODEL.SPECIFIC.PARMS <- list(verbose=FALSE,train.fraction=0.9) #NULL # Other model specific parameters


# train the model
set.seed(825)
l1_gbm_mdl <- do.call(train,c(list(x=L1FeatureSet$train$predictors,y=L1FeatureSet$train$y),
                            CARET.TRAIN.PARMS,
                            MODEL.SPECIFIC.PARMS,
                            CARET.TRAIN.OTHER.PARMS))

l1_gbm_mdl
cat("Average CV rmse:",mean(l1_gbm_mdl$resample$RMSE),"\n")

test_l1_gbm_yhat <- predict(l1_gbm_mdl,newdata = L1FeatureSet$test$predictors,type = "raw")
l1_gbm_submission <- cbind(Id=L1FeatureSet$test$id,SalePrice=exp(test_l1_gbm_yhat))
colnames(l1_gbm_submission) <- c("Id","SalePrice")

write.csv(l1_gbm_submission,file="l1_gbm_submission.csv",row.names=FALSE)


```

## Level 2 Model Blending

```{r blendModels}
l2_avg_yhat <- test_l1_nnet_yhat^0.9 * test_l1_gbm_yhat^0.1

l2_avg_submission <- cbind(Id=L1FeatureSet$test$id,SalePrice=exp(l2_avg_yhat))
colnames(l2_avg_submission) <- c("Id","SalePrice")

write.csv(l2_avg_submission,file="l2_avg_submission.csv",row.names=FALSE)
