...
```{r trainL1lasso}
# set caret training parameters
CARET.TRAIN.PARMS <- list(method="glmnet") 

CARET.TUNE.GRID <-  expand.grid(alpha=seq(0,1,0.01),
                                lambda=seq(0.3,0,-0.001))  # NULL provides model specific default tuning parameters
...
MODEL.SPECIFIC.PARMS <- list() #NULL # Other model specific parameters


# train the model
set.seed(825)
l1_lasso_mdl <- do.call(train,c(list(x=L1FeatureSet$train$predictors,y=L1FeatureSet$train$y),
                            CARET.TRAIN.PARMS,
                            MODEL.SPECIFIC.PARMS,
                            CARET.TRAIN.OTHER.PARMS))
...
#l1_lasso_mdl
cat("Best Tune:\n")
l1_lasso_mdl$bestTune
cat("Average CV rmse:",mean(l1_lasso_mdl$resample$RMSE),"\n")

test_l1_lasso_yhat <- predict(l1_lasso_mdl,newdata = L1FeatureSet$test$predictors,type = "raw")
l1_lasso_submission <- cbind(Id=L1FeatureSet$test$id,SalePrice=exp(test_l1_lasso_yhat))
colnames(l1_lasso_submission) <- c("Id","SalePrice")

write.csv(l1_lasso_submission,file="l1_lasso_submission.csv",row.names=FALSE)
...
```{r blendModels}
l2_avg_yhat <- test_l1_nnet_yhat^0.45 * test_l1_nnet_yhat^0.45 * test_l1_gbm_yhat^0.1
