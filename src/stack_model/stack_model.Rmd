...
```
This is an example of a stack generaliztion model.  A 5-fold cross-validation strategy is
used for model fitting.  The model is fitted to using the log of SalePrice as
the target variable.

Level 0 models consist of
...
time.data <- system.time(gbm_set <- llply(data_folds,trainOneFold,
                                     L0FeatureSet1))


test_y <- do.call(c,lapply(gbm_set,function(x){x$features$y}))
test_yhat <- do.call(c,lapply(gbm_set,function(x){x$features$yhat}))
rmse(test_y,test_yhat)
cat("Average test fold rmse:",mean(do.call(c,lapply(gbm_set,function(x){x$score}))))


# create test submission.
# A prediction is made by averaging the predictions made by using the models
# fitted for each fold.
test_yhat <- do.call(cbind,lapply(gbm_set,makeOneFoldTestPrediction,L0FeatureSet1))
...
CARET.TUNE.GRID <-  expand.grid(nrounds=1200, 
                                max_depth=10, 
                                eta=0.01, 
                                gamma=0.1, 
...
time.data <- system.time(xgb_set <- llply(data_folds,trainOneFold,
                                     L0FeatureSet2))


test_y <- do.call(c,lapply(xgb_set,function(x){x$features$y}))
test_yhat <- do.call(c,lapply(xgb_set,function(x){x$features$yhat}))
rmse(test_y,test_yhat)
cat("Average test fold rmse:",mean(do.call(c,lapply(xgb_set,function(x){x$score}))))


# create test submission.
# A prediction is made by averaging the predictions made by using the models
# fitted for each fold.
test_yhat <- do.call(cbind,lapply(xgb_set,makeOneFoldTestPrediction,L0FeatureSet2))
test_yhat <- apply(test_yhat,1,mean)

submission <- cbind(Id=L0FeatureSet2$test$id,SalePrice=exp(test_yhat))

write.csv(submission,file="xgb_sumbission.csv",row.names=FALSE)
