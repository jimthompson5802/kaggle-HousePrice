...
trainOneFold <- function(this_fold,feature_set) {
    ...
    ans <- list(fitted_mdl=fitted_mdl,
                score=score,
                predictions=data.frame(ID=test.data$ID,yhat=yhat,y=test.data$y))
...
# CV Error Estimate
test_y <- do.call(c,lapply(gbm_set,function(x){x$predictions$y}))
test_yhat <- do.call(c,lapply(gbm_set,function(x){x$predictions$yhat}))
...
CARET.TUNE.GRID <-  expand.grid(mtry=3*as.integer(sqrt(ncol(L0FeatureSet1$train$predictors))))

MODEL.SPECIFIC.PARMS <- list(verbose=0,num.trees=100) #NULL # Other model specific parameters
...
write.csv(l1_nnet_submission,file="l1_nnet_sumbission.csv",row.names=FALSE)
...
write.csv(l1_gbm_submission,file="l1_gbm_sumbission.csv",row.names=FALSE)
